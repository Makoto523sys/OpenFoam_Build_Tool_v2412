...
test('both upload inputs are visible on initial load and linked directly from the page header',()=>{const a=app();try{
  assert.match(a.d.title,/v8\.1$/);assert.equal(a.d.getElementById('enableAcceleration').checked,false);assert.equal(a.d.getElementById('waterRegionMode').value,'box');
  ...
  assert.equal(fs.readFileSync(require.resolve('../OpenFOAM_v2412_case_builder_v8_1.html'),'utf8'),fs.readFileSync(require.resolve('../OpenFOAM_v2412_case_builder_v3.html'),'utf8'));
}finally{a.close();}});
