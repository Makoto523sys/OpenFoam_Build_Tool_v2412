...
test('both upload inputs are visible on initial load and linked directly from the page header',()=>{const a=app();try{
  assert.match(a.d.title,/v8\.4$/);assert.equal(a.d.getElementById('enableAcceleration').checked,false);assert.equal(a.d.getElementById('waterRegionMode').value,'box');
