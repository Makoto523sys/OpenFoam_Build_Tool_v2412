...
function app(){
  ...
  const html=fs.readFileSync(require.resolve('../OpenFOAM_v2412_case_builder_v3.html'),'utf8').replace('init();\ninitWorkbench();','window.testAPI={Geometry,geometryParts,getPatches,allFaces,outputs:()=>outputs,blockingErrors,importGeometryFiles,restoreProject,makeZip,bcForField,cfg};\ninit();\ninitWorkbench();');
