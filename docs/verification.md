...
### v8.4への追記: 第4欄をSTLビュー右側へ移動

- 境界パッチ表・追加/削除操作・field手動調整を含む第4欄全体を、STLビュー右側の割り当てフォームと「領域と流れ方向」の下へ移動した。同じ要素とIDを移動し、一覧は1つだけ配置する。
- サイド欄の最小幅制約を解除し、幅の広い表は欄内で横スクロールする。field手動調整の操作は利用可能な幅で折り返す。既存の1000px以下の画面用レイアウトでは縦に並ぶ。
- DOM上の配置、重複IDがないこと、割り当て・削除・field操作を既存の自動テストで確認。117件成功、配布HTMLのビルド成功。実ブラウザでの描画目視は未実施。

### v8.4への追記: STL全体から背景範囲を自動設定

- 折りたたみ欄にあった背景範囲設定を、ビュー下の常時表示する黄色い欄へ移動した。余白は各側20%を初期値として変更できる。メッシュ用STLがない場合と読込中は操作を無効にする。
- 全メッシュ用STLの外接箱を余白付きで囲み、m座標・convertToMeters=1で設定する。薄い方向にも正の厚さを設ける。非表示・未選択のSTLを含め、初期水STLは含めない。
- 出力と背景枠を有効にして全体表示する。旧処理が保持点を外部流れ用に動かしていた動作を変更し、指定済みのlocationInMeshを保持する。セル分割数・パッチ条件も保持する。
- 追加5テストで、原点から離れた形状の各軸寸法、m / mm / cm / inch、STLエクスポートとblockMeshDictの包含関係、複数・非表示のSTL、初期水STLとの分離、平面の厚さ、任意余白、不正値・オーバーフロー時の無変更、作業保存/旧形式の復元を確認した。
- 配布HTMLのビルド成功、自動テスト122件成功。UI操作はNode/jsdom、全体表示は投影座標で確認した。実ブラウザでの描画目視とOpenFOAM実行は未実施。

### v8.4への追記: v2412起動・監視・実行前検査

- ユーザーのv2412 patch=260127通常起動ログを起点に、PIMPLE残差制御、圧力出口/逆流乱流量、solverInfo、流量監視、対称条件、層厚、無効なInfoSwitchを修正した。背景/STLの出所とSI寸法・入口流量目安を表示する。
- Allrunは既存ログで処理を省略せず、checkMeshを新規実行する。終了コード0だけで合格とせず、成功/正常終了マーカー、不合格行、検査前後のメッシュハッシュを照合する。実パッチと初期場・圧力条件・監視先の検査に失敗した場合は、並列分割とソルバーの前で停止する。
- 配布HTMLのビルド成功、Nodeテスト131件成功。このうち1件はPythonの21件のテストを実行し、実際の検査スクリプトと生成Allrunを動かす。OpenFOAMコマンド部分には明示したテスト用の代替コマンドを使用しており、実ソルバー実行ではない。
