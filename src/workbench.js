...
function refreshViewer(fit=false){if(viewer)viewer.setFaces(allFaces(),fit);if(workbenchReady){renderParts();updateSelection();}}
...
function updateWorkbenchStatus(){
  ...
  if(viewer){const scale=Number(val('stlScale'))||1;viewer.setGuide(['ami','mrf'].includes(mode)?rotorSurface().map(f=>({...f,v:f.v.map(v=>v.map(x=>x/scale))})):[]);}
...
function blockingErrors(){
  ...
  if(c.vof){const a=strictVector(val('waterBoxMin')),b=strictVector(val('waterBoxMax'));if(!a||!b||a.some((x,i)=>x>=b[i]))errs.push('VOF水領域の最小座標 < 最大座標を満たしてください。');}
  return [...new Set(errs)];
}
...
function restoreProject(data){
  ...
  for(const [id,value] of Object.entries(data.inputs||{})){const el=$(id);if(!el||el.type==='file'||!el.matches('input,select,textarea')||['previewText','previewSelect','applicationMirror'].includes(id))continue;if(el.type==='checkbox')el.checked=!!value;else el.value=String(value);}
  ...
  manualFields=Array.isArray(data.manualFields)?data.manualFields.filter(k=>Object.hasOwn(allFieldTemplates,k)):null;managedPatchNames=new Set();geometryHistory.length=0;refreshViewer(true);generate();setStatus('作業を復元しました。');
}
...
function initWorkbench(){
  ...
  viewer=new STLViewer($('stlCanvas'),pickFace);viewer.setFaces([],true);workbenchReady=true;
