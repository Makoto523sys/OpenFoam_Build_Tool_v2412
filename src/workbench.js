...
function blockingErrors(){
  ...
  if(mode!=='static'){
    ...
    if(mode==='refine'){if(patches.some(p=>p.purpose==='empty'))errs.push('界面適合細分化は厚み方向も分割するためempty境界には対応しません。3次元メッシュと対称境界等を設定してください。');if(!Number.isInteger(num('refineInterval'))||num('refineInterval')<1||!Number.isInteger(num('refineMax'))||num('refineMax')<1||!Number.isInteger(num('refineCells'))||num('refineCells')<1)errs.push('細分化間隔・レベル・セル数は正の整数です。');if(!Number.isFinite(num('refineLower'))||num('refineLower')<=0||num('refineLower')>=.5)errs.push('界面下限は0より大きく0.5未満です。');}
