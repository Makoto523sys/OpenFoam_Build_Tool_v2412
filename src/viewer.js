...
class STLViewer {
  ...
  setFaces(faces,fit=false) {
    this.faces=faces;
    if(fit){const b=faces.length?Geometry.bounds(faces):{min:[-1,-1,-1],max:[1,1,1]};this.center=b.min.map((x,i)=>(x+b.max[i])/2);this.radius=Math.max(Math.hypot(...Geometry.sub(b.max,b.min))/2,1e-8);this.zoom=1;this.pan=[0,0];}
    if(!this.gl)return;
